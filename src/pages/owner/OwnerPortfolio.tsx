...
import { useOwnerPortalData } from "@/hooks/useOwnerPortalData";
import { Card, CardContent } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Skeleton } from "@/components/ui/skeleton";
import { PoundSterling, Percent, BedDouble, Wallet, TrendingUp, TrendingDown, CalendarDays } from "lucide-react";
...
export default function OwnerPortfolio() {
  ...
  const prevYearLabel = String(currentYear - 1);
  ...
  const kpiCards = kpis ? [
    ...
    { label: "Your Payout", value: fmt(kpis.ownerPayout), icon: Wallet, prev: kpis.prevYearPayout },
  ] : [];
          ...
          {kpiCards.map((kpi) => (
            <Card key={kpi.label} className="border-border/30 bg-card/50 backdrop-blur-sm">
              <CardContent className="p-4 space-y-2">
                ...
                <TrendBadge current={parseFloat(kpi.value.replace(/[£%,]/g, "")) || 0} previous={kpi.prev} label={prevYearLabel} />
              </CardContent>
            </Card>
          ))}
        </div>
