...
DROP TRIGGER IF EXISTS trg_auto_create_clean_from_reservation ON public.reservations;

CREATE TRIGGER trg_auto_create_clean_from_reservation
AFTER INSERT OR UPDATE OF check_out, listing_id, status
ON public.reservations
FOR EACH ROW
EXECUTE FUNCTION public.auto_create_clean_from_reservation();

-- Backfill missing auto cleans for existing future reservations
INSERT INTO public.clean_tasks (
  listing_id, reservation_id, scheduled_date, status, source,
  is_same_day_turnaround, priority_level, checkout_time, task_type, priority
)
...
WHERE r.check_out >= CURRENT_DATE
  AND COALESCE(l.is_clean, true) = true
