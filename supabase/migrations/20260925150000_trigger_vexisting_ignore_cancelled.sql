-- Fix: the auto-create-clean trigger's v_existing lookup matched a clean for the
-- reservation in ANY status, including cancelled. So when an inquiry (or a cancelled
-- booking) later becomes confirmed, the trigger found the old cancelled row, ran its
-- UPDATE (which is itself guarded to completed/done and no-ops on a cancelled row),
-- and RETURNed — leaving NO live clean until the next scheduler run. The lookup must
-- ignore cancelled/canceled rows so a reinstated/confirmed booking creates a fresh
-- live clean immediately.
...
BEGIN
  ...
  BEGIN
    -- Ignore cancelled/canceled rows: a reinstated or newly-confirmed booking must
    -- get a fresh live clean, not silently reuse a dead row.
